...
public class ModelsController(CarDealerDbContext dbContext) : Controller //bütün database işlemlerini yaptığımız dbContext classını inject ettik.
{
    ...
    {
        var model = dbContext.Models.OrderBy(p => p.Name).ToList(); //veritabanından kullanıcının girdiği bilgileri Orderby(alfabe sırasına göre yukarıdan aşağıya(büyükten küçüğe) çektik.
    ...
    [HttpPost]
    public IActionResult Create(Model model) //post metoduyla gelen verileri make modelin içine koyduk. Kullanıcının doldurduğu veriler make modelin içinde geldi.
    {
    ...
    {
        var model = dbContext.Models.Find(id); //dbContext classının Models dbSetinin find methodu ile primary keyini gönderiyoruz o bize databaseden satırı bulup veriyor. Hangi markayı çekmek istediğimiz bulduk databaseden çektik View içinde göstericez.
    ...
    [HttpPost]
    public IActionResult Edit(Guid id, Model model) //url'deki id aynı kalır. modeli çağırdı.
    {
        dbContext.Models.Update(model);  // markayı güncelledikten sonraki hali.
